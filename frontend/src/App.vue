...
export default {
  ...
  methods: {
    ...
    async fetchUser () {
      const accessToken = localStorage.getItem('access');
      try {
        const { data: user } = await this.axios.get(`${import.meta.env
        .VITE_API_URL}/api/user/me/`,
    ...
    async refreshToken () {
      try {
        const { data } = await this.axios.post(`${import.meta.env
        .VITE_API_URL}/api/user/token/refresh/`, {
