...
export default {
  ...
  methods: {
    ...
    async fetchActors () {
      try {
        const { data: actors } = await
            this.axios.get(`${import.meta.env.VITE_API_URL}/api/cinema/actors/`, {
    ...
    async fetchGenres () {
      try {
        const { data: genres } = await
            this.axios.get(`${import.meta.env.VITE_API_URL}/api/cinema/genres/`, {
          headers: { Authorization: `Bearer ${this.token}` }
        });
        this.genres = genres;
      } catch (err) {
    ...
    async addMovie () {
      try {
        ...
        if (this.image) {
          ...
          await axios.post(`${import.meta.env.VITE_API_URL}/api/cinema/movies/${movie.id}/upload-image/`,
              data, imageConfig);
        }
