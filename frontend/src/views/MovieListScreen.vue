<template>
	<div v-if="active">
		<div class="multiselections">
			<custom-multiselect label="Select actors" :options="actors" @option-selected="handleActorSelection" @click="!actors.length && fetchActors()"></custom-multiselect>
			<custom-multiselect label="Select genres" :options="genres" @option-selected="handleGenreSelection" @click="!genres.length && fetchGenres()"></custom-multiselect>
		</div>
		<div class="movie-container">
			<movie-card v-for="(movie, index) in movies" :key="index" v-bind="movie" @click="handleMovieDetailsClick"></movie-card>
		</div>
		<add-btn v-if="isStaff" @click="handleMovieCreate"> </add-btn>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			hashHandler() {
				this.active = Boolean(!location.hash || location.hash.match('movies$') || location.hash.match(/#\/movies\/(\d+)/) || location.hash.match('/$'));
			},
