...
export default {
  ...
  methods: {
    async fetchActors () {
      try {
        const { data: actors } = await this.axios.get('cinema/actors//', {
    ...
    async fetchGenres () {
      try {
        const { data: genres } = await this.axios.get('cinema/genres/', {
          headers: { Authorization: `Bearer ${this.token}` }
        });
        this.genres = genres;
      } catch (err) {
    ...
    async fetchMovies () {
      ...
      try {
        const { data: movies } = await this.axios.get('cinema/movies//', {
          headers: { Authorization: `Bearer ${this.token}` },
          params
        });
