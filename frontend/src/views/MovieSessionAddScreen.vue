<template>
  <div v-if="active && isStaff" class="movie-session-container">
    ...
    <div class="container">
      <custom-select label="Movie" :options="movieOptions"
                     @option-selected="handleMovieSelection"></custom-select>
      <custom-select label="Cinema Hall" :options="cinemaHalls"
                     @option-selected="handleHallSelection"></custom-select>
      <div class="date-container">
        <date-picker v-model="date"></date-picker>
        <time-picker v-model="time"></time-picker>
      </div>
    </div>
    <action-button label="Submit" @click="addMovieSession"
                   :disabled="!selectedHallId || !selectedMovieId"></action-button>
  </div>
</template>
